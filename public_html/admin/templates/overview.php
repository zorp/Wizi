<?php
    ...
    if( !$selected_overview ) $selected_overview = $_POST["selected_overview"];
		if( !$selected_overview ) $selected_overview = $_GET["selected_overview"];
    if( !$selected_overview ) $selected_overview = "draft";
    if( !$interval ) $interval = $_POST["interval"];
		if( !$interval ) $interval = $_GET["interval"];
    ...
    $states = array("draft"=>"Documents with draft version",
										"publish"=>"Documents publishing state",
    ...
    $wizitorsDescription.= " visiting the site within the specified interval ";
		...
		$descriptions = array("draft"=>"<img src=../tree/graphics/doc_gray.gif /> Show which documents has a draft version not yet published.",
													"publish"=>"
		<img src=../tree/graphics/doc.gif /> = Published document.<br />
		<img src=../tree/graphics/doc_gray.gif /> = Unpublished document.<br />
		<img src=../tree/graphics/doc_unpub.gif /> = Document scheduled to unpublish.<br />
		<img src=../tree/graphics/doc_pub.gif /> = Document scheduled to become published.<br />
		<img src=../tree/graphics/doc_unpub_pub.gif /> = Document scheduled to become published and then unpublished.<br /><br />
		Click on the icons to the left, to make changes to a document",
		
		"nav"=>"<img src=../tree/graphics/doc.gif /> All ducuments with this icon is set to display on the website navigation.<br /><br />Click on the icon to edit the document properties",
    
		"title"=>"<img src=../tree/graphics/doc_gray.gif /> All documents with this icon has no title defined.<br /><br/>Click on the icon to edit the document properties",
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" class="color1">
	<tr>
       ...
       <option value="draft" <?=( $selected_overview =='draft')?"selected":""?>>Documents with a draft version</option>
							<option value="publish" <?=( $selected_overview =='publish')?"selected":""?>>Published - unpublished</option>
       <option value="nav" <?=( $selected_overview =='nav')?"selected":""?>>Documents visible on navigation</option>
       <option value="title" <?=( $selected_overview =='title')?"selected":""?>>Documents without a title</option>
       <option value="docs" <?=( $selected_overview =='docs')?"selected":""?>>Documents with dependencies</option>
       ...
       <option value="wizitors" <?=( $selected_overview =='wizitors')?"selected":""?>>Online visitors</option>
						...
						<table width="100%" cellpadding="0" cellspacing="0" border="0">															
                          ...
                          <!--</a>-->
                            ...
                            <?if( $table == 'tree' ):?>
                                ...
                                ?>
                                <a href="../documents/index.php?id=<?=$nodes[$i]["id"]?>&pane=<?=$documentPane?>&referer=<?=urlencode( $_SERVER["PHP_SELF"] ."?pane=".$pane."&selected_overview=". $selected_overview) ?>"><img src="../tree/graphics/doc<?=$iconPostFix?>.gif" alt="<?=$alt?>" border="0"/></a>
                            <?else:?>
                              <?
                                  $iconPostFix='';
                                  if( !$filter[ $nodes[$i]["id"] ]["state"] )
                              ...
                              ?>
                              <a href="../media/index.php?id=<?=$nodes[$i]["id"]?>&pane=dependencies&referer=<?=urlencode( $_SERVER["PHP_SELF"] ."?pane=". $pane ."&selected_overview=". $selected_overview )?>"><img src="../mediatree/graphics/file_icons/<?=$format?><?=$iconPostFix?>.gif" alt="<?=$alt?>" border="0"/></a>
                            <?endif?>
                        </span>
											</td>
											<?//==================SPACER CELL============================?>
            ...
            <?if( $selected_overview == 'wizitors' ):?>
              <br><br>
              <input type="button" value="Refresh" class="knapgreen" onclick="document.location.href='<?=$_SERVER["PHP_SELF"]?>?interval=<?=$overview->interval?>&pane=overview&selected_overview=wizitors'">
            <?endif?>
          </td>
				</tr>
			</table>
