...
class document
{
    ...
    var $topic;
		...
		/**
     * Integer to decide if comment field is shown
     * @type int 
     */
    var $showcomment;
         ...
         for( $i = 0; $i < $n; $i++ )
         {
          ...
          $sql ="SELECT 
                  ...
                  doc.content     AS content,
									LEFT( doc.content, 300 ) AS summary
                FROM 
                  ".$this->p."search  AS s,
                  ".$this->p."tree    AS doc 
                WHERE 
    ...
    function setTopic( $topic )
    {
		...
		function setComment( $showcomment )
    {
        $this->showcomment = ( $showcomment == 'on' )? 'y': 'n';
        $sql = "UPDATE
                    ".$this->p."tree
                SET
    ...
    function setFromnews( $d=0, $m=0, $y=0 )
    {
        ...
        if( $d )
        {
            ...
            $sql.= "CONCAT( '$y-$m-$d ', HOUR( 00 ) ,':', MINUTE( 00 ) ,':', SECOND( 00 ) )";
        }
        else
        {
            $sql.= "NULL";
    ...
    function translateLinks( $string )
    {
        ...
        $string  = ereg_replace( $pattern, $replace, $string );
				...
				//translate checkurl links
        $pattern = 'href="'."[^\"]*checkUrl.php\?";
    		$replace = 'href="checkUrl.php?';

        $string  = ereg_replace( $pattern, $replace, $string );
				
				//translate intern media links
        $pattern = 'href="'."[^\"]*media.php\?";
    		$replace = 'href="media.php?';
    ...
    function translateLinksBack( $string )
    {
        ...
        $pattern = 'src="'."[^\"]*media\/([0-9]*\....)";
        $replace = 'src="'.$this->dba->rootpath.'media/'."\\1";

        $string  = ereg_replace( $pattern, $replace, $string );
				
				$pattern = 'VALUE="'."[^\"]*media\/([0-9]*\....)";
        $replace = 'VALUE="'.$this->dba->rootpath.'media/'."\\1";
		...
		function addInclude( $doc, $type )
    {
		...
		function addIncludeExternal( $url, $type )
    {
			if( !trim( $url ) || !trim( $type ) ) return;
  ...
  function getNestedIncludes()
  {
    ...
    function getAllIncludes( $id, $path )
    {
        ...
        for( $i = 0; $i < $n; $i++ )
        {
            ...
            $this->allIncludes[ $anchorsN ]["id"]      = $record["incid"];
						$this->allIncludes[ $anchorsN ]["incurl"]  = $record["incurl"];
    ...
    function getPublished( $results )
    {
      ...
      if( $ids )
      {
        ...
        for( $i = 0; $i < $n; $i++ )
        {
          ...
          $securityCheck = $this->getRole($results[ $rec[0] ]["id"]);
					if ($securityCheck["roleId"] != 1 && $securityCheck["roleId"] != 2 && $securityCheck["roleId"] != 3 ) $newresults[ $i ] = $results[ $rec[0] ];
        }
      }
		...
		function getRole($id)
    {
		  ...
		  $sql = "SELECT
                ...
                AND
                    roleconstrain.doc = ". $id ."
                ORDER BY
                    constrain.id DESC 
                LIMIT 0,1";
        $role = $this->dba->singleArray( $sql );
	...
	function loadDraftProperties()
    {
        ...
        $this->name         = stripslashes ( $record["name"] );
        $this->title        = stripslashes ( $record["drafttitle"] );
        $this->heading      = stripslashes ( $record["draftheading"] );
        $this->nav          = $record["nav"];
        $this->timepublish  = $record["timepublish"];
        $this->timeunpublish= $record["timeunpublish"];
        $this->description  = stripslashes ( $record["draftdescription"] );
        $this->meta         = stripslashes ( $record["draftmeta"] );
        $this->template     = $record["template"];
        $this->layout       = $record["layout"];
        $this->news         = $record["news"];
        $this->topic        = $record["topic"];
		...
		function setDraftTitle( $title )
    {
        ...
        $sql = "UPDATE
                    ".$this->p."tree
                SET
                    drafttitle ='". addslashes( $this->title ) ."'
                WHERE 
                    id=". $this->id;
        $this->dba->exec($sql);
    }
    function setDraftHeading( $heading )
    {
        $this->heading = trim( $heading );
        ...
        $sql = "UPDATE
                    ".$this->p."tree
                SET
                    draftheading ='". addslashes( $this->heading ) ."'
                WHERE 
                    id=". $this->id;
        $this->dba->exec($sql);
    }
    function setDraftDescription( $description )
    {
        $this->description = trim( $description );
        ...
        $sql = "UPDATE
                    ".$this->p."tree
                SET
                    draftdescription ='". addslashes( $this->description ) ."'
                WHERE 
                    id=". $this->id;
        $this->dba->exec($sql);
    }
    function setDraftMeta( $meta )
    {
        ...
        $sql = "UPDATE
                    ".$this->p."tree
                SET
                    draftmeta ='". addslashes( $this->meta ) ."'
                WHERE 
                    id=". $this->id;
        $this->dba->exec($sql);
    }
        ...
        else
        {
            ...
            $this->content = $content;
				...
				$sql = "UPDATE
                    ".$this->p."tree
                SET
                    draftcontent = '". addslashes( trim( $content ) )."'
                WHERE
                    id=". $this->id;
        $this->dba->exec( $sql );
				$this->draftEdited();
    ...
    function getDraftTranslatedContent()
    {
        return $this->translateLinksBack( $this->getDraftContent() );
    }
