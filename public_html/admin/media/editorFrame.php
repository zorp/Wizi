...
if ( $editor->getAction() == "save" )
{
	$newImgInfo = @getimagesize($imageInfo['fullpath']);
	$newImgSize = @filesize( $imageInfo['fullpath'] );
	$height = $newImgInfo[1];
	$width = $newImgInfo[0];
	...
	echo "<script>";
	echo "var url = parent.opener.window.location.href;";
	echo "parent.opener.window.location.href = url+'&width=".$width."&height=".$height."&size=".$newImgSize."';</script>";
	echo "</script>";
}
...
<? if ($editor->isGDEditable() == -1) { ?>
	<div style="text-align:center; padding:10px;"><span class="error">The server report that GIF format is not supported, you can try to edit image anyway but it may not work.</span></div>
<? } ?>
