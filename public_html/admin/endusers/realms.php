<?
    require_once("../util/endUserRealms.php");
    ...
    $realm  = new endUserRealms( $dba, $role->id );
    if( $docId ) $realm->toogleConstrainsForDoc( $docId );
    $realms4role = $realm->getDocConstrainsForRole();
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	...
	<tr>
		<td class="header">Realms for restriction "<?=$role->name?>"</td>
	</tr>
	<tr>
	    	...
	    	<table cellpadding="0" cellspacing="0" border="0">
	    		<tr>
	      		<td valign="top" class="tdpadtext" style="padding-right:15px;">
							Select restriction:<br/>
							<select name="id" class="select_list" onchange="document.tree.submit()">
								<?for( $i = 0; $i< count( $rolesList ); $i++ ):?>
									<option value="<?=$rolesList[$i]["id"]?>" <?if( $rolesList[$i]["id"] == $id ) echo "selected";?> ><?=$rolesList[$i]["name"]?></option>
								<?endfor?>
							</select>
						</td>
							...
							<table width="250" cellpadding="0" cellspacing="0" border="0" class="tdpadtext">
								...
								<tr>
								    ...
								    <table cellpadding="0" cellspacing="0" border="0">
							          ...
							          <?//==================NODE ICON CELL============================?>
							          <td><a href="#" onclick="selectNode( <?=$nodes[$i]["id"]?> )" title="Select" class="nodeName" onfocus="if(this.blur)this.blur();"><img src="../tree/graphics/doc<?=( $realms4role[ $nodes[$i]["id"] ] )?"":"_gray"?>.gif" alt="Toggle" border="0"/></a></td>
												<?//==================SPACER CELL============================?>
												<td><img src="../tree/graphics/space.gif" width="5" height="10" alt="space"\></td>
												<?//==================ITEM NAME CELL============================?>
								        <td><a href="#" onclick="selectNode( <?=$nodes[$i]["id"]?> )" title="Select" class="nodeName<?=( $realms4role[ $nodes[$i]["id"] ] )?"":"_gray"?>" onfocus="if(this.blur)this.blur();"><?=$nodes[$i]["name"] ?></a></td>
											</tr>
										</table>
									</td>
								</tr>
